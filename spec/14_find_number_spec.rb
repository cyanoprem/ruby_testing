...
describe FindNumber do
  ...
  describe '#game_over?' do
    ...
    context 'when guess and random_number are equal' do
      let(:correct_number) { double('random_number', value: 7) }
      subject(:game_complete) { described_class.new(0, 9, correct_number, 7) }
      ...
      it 'is game over' do
        game_end = game_complete.game_over?
        expect(game_end).to eq(true)
      end
    end
    ...
    context 'when guess and random_number are not equal' do
      let(:wrong_number) { double('random_number', value: 3) }
      subject(:game_continue) { described_class.new(0, 9, wrong_number, 7) }
      it 'is not game over' do
        game_end = game_continue.game_over?
        expect(game_end).not_to eq(true)
      end
    end
  end
  ...
  describe '#update_range' do
    ...
    context 'when the guess is 7, min=5, and max=8' do
      ...
      it 'updates min to the same value as max' do
        low_guess_game.update_range
        expect(low_guess_game.min).to eq(8)
      end
